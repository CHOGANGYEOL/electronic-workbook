...
export const UC_2023_04: Question = {
	questionId: 202304,
	title: '2023 상시 4회차',
	contents: [
		{
		...
		},
		{
			...
			answer: ['기본 게이트웨이', '서브넷 마스크', 'IP 주소', 'DHCP 서버 주소'],
			correct: 4,
		},
		{
			contentId: 4,
		...
		{
			contentId: 5,
			question: '다음 중 컴퓨터에서 문자를 표현하는 코드 체계에 대한 설명으로 옳지 않은 것은?',
			answer: [
				'BCD 코드는 64가지의 문자를 표현할 수 있으나 영문 소문자는 표현 불가능하다.',
				'EBCDIC 코드는 BCD 코드를 확장한 코드체계로 256가지의 문자를 표현할 수 있다.',
		...
		},
		{
			...
			answer: [
				'중앙처리장치와 주기억장치 사이에 위치하여 컴퓨터의 처리 속도를 향상 시키는 역할을 한다.',
				'CPU가 데이터를 처리하는 동안 미리 CPU가 필요로 하는 데이터를 저장해 두는 기억장치이다.',
				'보조기억장치의 일부를 주기억장치처럼 사용하는 메모리 사용 기법으로 주기억장치 보다 큰 프로그램을 로드하여 실행할 경우에 유용하다.',
		...
		},
		{
			...
			answer: [
				...
				'고급 언어는 번역 과정이 없어 보다 편리하게 프로그래밍할 수 있다.',
		...
		},
		{
			...
			answer: [
				...
				'PNG: 트루컬러의 지원과 투명색 지정이 가능하다.',
			],
			correct: 1,
		},
		{
			contentId: 17,
			question: '다음 중 컴퓨터 통신과 관련하여 P2P 방식에 관한 설명으로 옳은 것은?',
			answer: [
		...
		{
			contentId: 24,
			question: '다음 중 시트 보호에 대한 설명으로 옳지 않은 것은?',
			answer: [
		...
		},
		{
			contentId: 25,
			question:
				'[A2:E10] 영역에 ‘판매량’이 40 이상이고, ‘상태’가 “양호”면 배경색을 ‘노랑’으로 설정하는 [조건부 서식]을 지정하려고 한다. 다음 중 [조건부 서식]의 수식 입력란에 입력해야 할 수식으로 옳은 것은?',
		...
		{
			contentId: 26,
			question: '다음 중 참조의 대상 범위로 사용하는 이름 정의 시 이름의 지정 방법에 대한 설명으로 옳지 않은 것은?',
			answer: [
		...
		},
		{
			contentId: 29,
			question:
				'[C2:E5] 영역을 선택한 후 배열 수식으로 한 번에 금액을 구하려고 한다. 다음 중 이를 위한 수식으로 옳은 것은? (금액 = 수량 * 단가)',
		...
		},
		{
			...
			answer: [
				'주식형: 고가, 저가, 종가 등의 주식 거래 가격을 바탕으로 차트를 작성한다.',
				'분산형 차트: 여러 데이터 계열에 있는 숫자 값 사이의 관계를 보여 주거나 두 개의 숫자 그룹을 xy 좌표로 이루어진 하나의 계열로 표시할 때 사용된다.',
				'거품형: 데이터 값이 두 개인 경우에만 사용할 수 있으며 첫 번째 값이 X축, 두 번째 값이 데이터 표식의 크기로 사용된다.',
				'표면형 차트: 두 개의 데이터 집합에서 최적의 조합을 찾을 때 사용된다.',
			],
			correct: 3,
		},
		{
			contentId: 31,
			question:
				'아래의 시트에서 [I2:I5] 영역에 [B2:E14] 영역의 표를 참조하는 배열 수식을 사용하여 지점별 총대출금액을 구하였다. 다음 중 [I2:I5] 영역을 블록으로 지정한 후 수식을 입력할 경우 수식 입력줄에 표시된 함수식으로 옳은 것은?',
			questionBody: <img src={IMAGES.Img_31} />,
			answer: [
				'{=SUMIF(D2:D14, H2, E2:E14)}',
				'{=SUMIF($D$2:$D$14=H2, $E$2:$E$14, 1)}',
				'{=SUMIF(D2:D14, H2:H5, E2:E14)}',
		...
		{
			contentId: 35,
			question: '다음 중 외부 데이터베이스의 데이터를 가져오기 위한 쿼리 마법사의 설명으로 옳지 않은 것은?',
			answer: [
		...
		},
		{
			contentId: 36,
			question:
				'아래의 워크시트에서 ‘성명’이 두 글자이고 실적이 전체 실적의 평균보다 큰 데이터를 필터링하고자 한다. 다음 중 고급 필터 실행을 위한 조건의 입력 값으로 옳은 것은?',
			questionBody: <img src={IMAGES.Img_36} />,
			answer: [
				<table key={'UC_2023_04_36'}>
					<tr>
						<th>성명</th>
						<th>실적</th>
					</tr>
		...
		{
			contentId: 42,
			question: '다음 중 하위 폼에서 새로운 레코드를 추가하려고 할 때 설정해야 할 폼 속성은?',
			answer: [
				'‘필터 사용’을 예로 설정한다.',
				'‘추가 가능’을 예로 설정한다.',
				'‘편집 가능’을 예로 설정한다.',
		...
		{
			contentId: 44,
			question: '다음 중 [관계 편집] 대화상자에 대한 설명으로 옳지 않은 것은?',
			answer: [
		...
		},
		{
			...
			answer: ['외부 스키마', '서브 스키마', '개념 스키마', '내부 스키마'],
			correct: 3,
		},
		{
			contentId: 49,
			...
			answer: ['SELECT 문', 'GROUP BY 절', 'HAVING 절', 'ORDER BY 절'],
			correct: 4,
		},
		{
			contentId: 50,
			question:
				'하위 보고서를 만들 때 아래의 조건을 만족하면 주 보고서와 하위 보고서가 자동으로 연결되어 목록에 표시된다. 다음 중 괄호에 들어갈 단어를 순서대로 바르게 나열한 것은?',
			questionBody: (
				<RequestBody>
					<ul>
						<li>주 보고서와 하위 보고서에서 사용되는 테이블/쿼리 등이 (a ) 관계로 설정된 경우</li>
						<li>
							주 보고서는 (b )을(를) 가진 테이블/쿼리를 사용하고, 하위 보고서는 (c )와(과) 같거나 호환되는 데이터 형식을
							가진 필드가 포함된 테이블/쿼리를 사용할 경우
						</li>
					</ul>
				</RequestBody>
			),
			answer: [
				'a-일대일, b-필드, c-기본키',
				'a-일대다, b-기본키, c-기본키 필드',
				'a-일대일, b-레코드, c-기본키 필드',
				'a-일대다, b-기본키 필드, c-필드',
			],
			correct: 2,
		},
		...
		{
			contentId: 55,
			question: '다음 중 기본 폼과 하위 폼을 연결하기 위한 기본 조건에 대한 설명으로 옳지 않은 것은?',
			answer: [
		...
		},
		{
			...
			answer: [
				...
				'매크로는 주로 컨트롤의 이벤트에 연결하여 사용하며, 폼 개 체 내에서만 사용할 수 있다.',
			],
			correct: 4,
		},
	],
